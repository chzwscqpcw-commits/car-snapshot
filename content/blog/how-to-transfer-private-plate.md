...
- **Both vehicles need a valid MOT** (if over 3 years old) — check this first using our [MOT check](/mot-check)
...
Before starting, verify that both vehicles are taxed (or SORN'd) and have valid MOTs. Use our [tax check](/tax-check) and [MOT check](/mot-check) to confirm. The DVLA will reject the transfer if either vehicle fails these requirements.
...
**Not checking MOT and tax status** — The DVLA requires both vehicles to be taxed (or SORN'd) with valid MOTs. A lapsed MOT will block the transfer. Check status with our [free car check](/car-check) before applying.
...
Transferring a private plate is straightforward once you understand the process — it's essentially an £80 application to the DVLA. The most important things are to check that both vehicles meet the [MOT](/mot-check) and [tax](/tax-check) requirements before applying, and to put plates on retention before selling a vehicle.

Start by running a [free car check](/car-check) on both vehicles to confirm their status, then apply online through the DVLA for the fastest processing.
