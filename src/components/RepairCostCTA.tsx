...
export default function RepairCostCTA({
  ...
  // Path A — reg lookup (our unique angle: routes back into the check tool)
  const regLookup = (
  ...
  // Path B — BookMyGarage affiliate CTA
  const bmgCta = (
    <div className="flex items-start gap-3">
  ...
  return (
    <div className="my-8 rounded-xl border border-blue-800/40 bg-gradient-to-br from-blue-950/40 via-slate-900/60 to-slate-900/60 p-4 sm:p-5">
      {hideRegLookup ? (
        bmgCta
      ) : (
        // Two distinct choices — each in its own inset card, split by an OR
        // badge, so the reg-lookup and the BookMyGarage link never read as one
        // continuous flow.
        <div className="flex flex-col gap-1">
          <div className="rounded-lg border border-slate-700/50 bg-slate-950/40 p-4 sm:p-5">
            {regLookup}
          </div>

          <div className="flex items-center gap-3 py-1 text-xs text-slate-500">
            <span className="h-px flex-1 bg-slate-800" />
            <span className="grid h-8 w-8 place-items-center rounded-full border border-slate-700 bg-slate-900 font-semibold tracking-wide text-slate-400">
              OR
            </span>
            <span className="h-px flex-1 bg-slate-800" />
          </div>
